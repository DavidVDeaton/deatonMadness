let calebPicks = {
    name: "caleb",
    medal: "silver",
    score: 0,
    picks: [
        "01s","09s","05s","04s","11s","14s","10s","15s",
        "01e","09e","12e","13e","06e","03e","10e","02e",
        "01m","08m","05m","13m","11m","14m","10m","02m",
        "01w","08w","05w","13w","06w","14w","10w","02w",
        "01s","04s","11s","10s",
        "01e","12e","03e","02e",
        "01m","05m","11m","02m",
        "01w","05w","06w","02w",
        "01s","11s",
        "01e","02e",
        "05m","02m",
        "01w","02w",
        "01s","01e","02m","01w",
        "01e","01s","01e"
    ]  
};

let suzyPicks = {
    name: "suzy",
    medal: "",
    score: 0,
    picks: [
        "01s","09s","12s","04s","06s","03s","07s","02s",
        "01e","08e","05e","04e","11e","03e","10e","02e",
        "01m","09m","05m","04m","11m","03m","07m","02m",
        "01w","09w","12w","04w","11w","03w","10w","02w",
        "01s","04s","03s","07s",
        "01e","05e","03e","02e",
        "09m","05m","03m","07m",
        "09w","04w","03w","02w",
        "01s","03s",
        "01e","02e",
        "05m","03m",
        "09w","02w",
        "03s","02e","05m","02w",
        "02e","05m","02e"
    ]  
};

let renPicks = {
    name: "ren",
    medal: "",
    score: 0,
    picks: [
        "01s","08s","05s","13s","06s","14s","07s","02s",
        "01e","09e","12e","13e","06e","03e","07e","02e",
        "01m","09m","05m","13m","06m","03m","10m","02m",
        "01w","09w","05w","04w","11w","03w","07w","02w",
        "01s","05s","14s","02s",
        "01e","05e","06e","02e",
        "01m","13m","06m","02m",
        "09w","05w","03w","07w",
        "01s","14s",
        "13e","02e",
        "13m","06m",
        "09w","03w",
        "01s","02e","13m","09w",
        "02e","01s","01s"
    ]  
};

// let hyrumPicks = {
//     name: "hyrum",
//     medal: "",
//     score: 0,
//     picks: [
//         "01s","08s","05s","04s","06s","03s","10s","02s",
//         "01e","08e","05e","04e","06e","03e","07e","02e",
//         "01m","09m","05m","04m","06m","03m","07m","02m",
//         "01w","08w","05w","04w","06w","03w","10w","02w",
//         "01s","04s","03s","02s",
//         "01e","04e","03e","02e",
//         "01m","05m","06m","02m",
//         "01w","04w","03w","02w",
//         "01s","02s",
//         "04e","03e",
//         "01m","02m",
//         "01w","02w",
//         "01s","04e","02m","01w"
//         // "04e","02m","02m"
//     ]  
// };

let reedPicks = {
    name: "reed",
    medal: "",
    score: 0,
    picks: [
        "01s","08s","12s","13s","06s","03s","07s","02s",
        "01e","09e","12e","13e","11e","14e","07e","02e",
        "01m","08m","12m","13m","06m","14m","07m","02m",
        "01w","08w","12w","13w","11w","14w","10w","02w",
        "01s","13s","06s","07s",
        "01e","12e","14e","02e",
        "08m","12m","06m","07m",
        "01w","12w","11w","02w",
        "01s","07s",
        "01e","14e",
        "12m","06m",
        "12w","02w",
        "01s","01e","12m","02w",
        "02w","01s","02w"
    ]  
};

let paigePicks = {
    name: "paige",
    medal: "",
    score: 0,
    picks: [
        "01s","08s","05s","13s","06s","14s","07s","02s",
        "01e","08e","05e","13e","06e","03e","10e","02e",
        "01m","09m","12m","13m","06m","03m","07m","02m",
        "16w","08w","05w","04w","06w","03w","07w","02w",
        "01s","13s","14s","07s",
        "01e","05e","06e","02e",
        "01m","13m","06m","02m",
        "16w","05w","06w","02w",
        "01s","07s",
        "01e","02e",
        "01m","02m",
        "05w","02w",
        "01s","02e","02m","02w",
        "02e","02m","02e"
    ]  
};

let noahPicks = {
    name: "noah",
    medal: "",
    score: 0,
    picks: [
        "01s","09s","05s","04s","06s","03s","07s","02s",
        "01e","09e","05e","04e","11e","03e","10e","02e",
        "01m","08m","05m","04m","11m","03m","07m","02m",
        "01w","08w","12w","04w","11w","03w","10w","02w",
        "01s","05s","03s","02s",
        "01e","04e","03e","02e",
        "01m","05m","03m","02m",
        "01w","04w","03w","02w",
        "01s","03s",
        "01e","03e",
        "01m","03m",
        "01w","02w",
        "01s","03e","01m","01w",
        "01w","01s","01w"
    ]  
};

let davidPicks = {
    name: "david",
    medal: "",
    score: 0,
    picks: [
        "01s","09s","05s","04s","06s","03s","10s","02s",
        "01e","09e","05e","04e","06e","03e","07e","02e",
        "01m","09m","12m","13m","06m","03m","07m","02m",
        "01w","09w","12w","04w","06w","03w","10w","02w",
        "01s","04s","03s","02s",
        "01e","04e","03e","07e",
        "01m","12m","06m","02m",
        "01w","12w","03w","10w",
        "04s","03s",
        "01e","03e",
        "01m","02m",
        "01w","03w",
        "03s","01e","02m","01w",
        "01e","02m","02m"
    ]  
};

let jamesPicks = {
    name: "james",
    medal: "",
    score: 0,
    picks: [
        "01s","08s","12s","04s","11s","14s","10s","15s",
        "01e","09e","12e","04e","11e","14e","07e","15e",
        "01m","09m","05m","13m","06m","03m","07m","15m",
        "01w","08w","12w","13w","06w","14w","10w","02w",
        "08s","12s","11s","15s",
        "01e","12e","14e","07e",
        "01m","13m","06m","07m",
        "01w","12w","06w","02w",
        "12s","11s",
        "01e","07e",
        "13m","06m",
        "12w","02w",
        "12s","01e","06m","02w",
        "02w","12s","12s"
    ]  
};

let mabryPicks = {
    name: "mabry",
    medal: "gold",
    score: 0,
    picks: [
        "16s","09s","05s","04s","11s","14s","10s","02s",
        "01e","09e","12e","04e","06e","14e","07e","15e",
        "01m","09m","05m","13m","11m","03m","10m","02m",
        "16w","09w","05w","04w","06w","03w","07w","02w",
        "16s","04s","14s","02s",
        "01e","12e","14e","07e",
        "01m","05m","03m","02m",
        "16w","05w","06w","02w",
        "16s","14s",
        "12e","14e",
        "01m","03m",
        "05w","06w",
        "16s","12e","03m","06w",
        "12e","03m","03m"
    ]  
};

let aprilPicks = {
    name: "april",
    medal: "",
    score: 0,
    picks: [
        "01s","09s","05s","04s","11s","03s","07s","02s",
        "01e","09e","12e","04e","06e","03e","10e","02e",
        "01m","08m","05m","04m","11m","03m","10m","15m",
        "01w","08w","05w","13w","11w","03w","07w","02w",
        "01s","05s","03s","07s",
        "01e","04e","06e","10e",
        "01m","05m","11m","10m",
        "08w","05w","11w","02w",
        "01s","03s",
        "01e","06e",
        "05m","11m",
        "05w","02w",
        "01s","01e","05m","02w",
        "01e","01w","01e"
    ]
};

let davePicks = {
    name: "dave",
    medal: "",
    score: 0,
    picks: [
        "01s","08s","12s","04s","06s","03s","10s","02s",
        "01e","09e","12e","04e","06e","03e","10e","02e",
        "01m","08m","12m","13m","11m","03m","07m","02m",
        "01w","09w","12w","04w","11w","03w","10w","02w",
        "08s","04s","03s","02s",
        "01e","04e","03e","10e",
        "01m","12m","11m","02m",
        "01w","04w","03w","02w",
        "04s","03s",
        "01e","03e",
        "01m","02m",
        "01w","02w",
        "03s","01e","02m","01w",
        "01e","02m","01e"
    ]
};

let katiePicks = {
    name: "katie",
    medal: "bronze",
    score: 0,
    picks: [
        "01s","09s","05s","04s","06s","03s","07s","02s",
        "01e","09e","05e","04e","06e","03e","07e","02e",
        "01m","08m","05m","04m","06m","03m","07m","15m",
        "01w","09w","12w","04w","06w","03w","07w","02w",
        "01s","04s","06s","02s",
        "01e","04e","06e","02e",
        "08m","05m","03m","15m",
        "09w","04w","03w","02w",
        "04s","02s",
        "01e","02e",
        "05m","03m",
        "04w","02w",
        "04s","01e","03m","02w",
        "02w","04s","04s"
    ]
};

let mattPicks = {
    name: "matt",
    medal: "",
    score: 0,
    picks: [
        "01s","09s","12s","04s","11s","03s","10s","02s",
        "01e","09e","05e","04e","06e","14e","10e","02e",
        "01m","09m","05m","04m","11m","03m","07m","02m",
        "01w","09w","12w","13w","06w","03w","07w","02w",
        "01s","04s","03s","02s",
        "01e","05e","06e","02e",
        "01m","04m","03m","02m",
        "01w","12w","03w","07w",
        "04s","03s",
        "01e","02e",
        "01m","02m",
        "01w","03w",
        "03s","02e","01m","01w",
        "01w","01m","01m"
    ]
};

let susanPicks = {
    name: "susan",
    medal: "",
    score: 0,
    picks: [
        "01s","08s","12s","04s","06s","03s","07s","02s",
        "01e","08e","05e","04e","06e","03e","10e","02e",
        "01m","09m","12m","04m","11m","03m","07m","02m",
        "01w","09w","12w","04w","11w","03w","10w","02w",
        "01s","04s","03s","02s",
        "01e","04e","03e","02e",
        "01m","04m","11m","02m",
        "01w","04w","11w","02w",
        "01s","03s",
        "01e","03e",
        "01m","02m",
        "01w","02w",
        "03s","01e","01m","02w",
        "01e","01m","01e"
    ]
};

let dennisPicks = {
    name: "dennis",
    medal: "",
    score: 0,
    picks: [
        "01s","09s","05s","04s","11s","03s","07s","02s",
        "01e","09e","05e","04e","06e","03e","10e","02e",
        "01m","09m","05m","04m","11m","03m","07m","02m",
        "01w","09w","12w","04w","11w","03w","10w","02w",
        "01s","05s","03s","02s",
        "01e","04e","03e","02e",
        "01m","05m","03m","02m",
        "01w","04w","11w","02w",
        "01s","02s",
        "01e","02e",
        "01m","02m",
        "04w","02w",
        "01s","01e","02m","02w",
        "01e","01s","01e"
    ]
};

let dawsonPicks = {
    name: "dawson",
    medal: "",
    score: 0,
    picks: [
        "01s","08s","05s","04s","06s","03s","07s","02s",
        "01e","09e","05e","04e","06e","03e","10e","02e",
        "01m","09m","05m","13m","06m","03m","10m","02m",
        "01w","09w","05w","04w","06w","03w","07w","02w",
        "01s","05s","03s","02s",
        "01e","04e","03e","02e",
        "01m","05m","06m","02m",
        "01w","04w","03w","02w",
        "01s","03s",
        "01e","02e",
        "01m","02m",
        "01w","02w",
        "01s","01e","01m","02w",
        "01e","01m","01e"
    ]
};

let savannahPicks = {
    name: "savannah",
    medal: "",
    score: 0,
    picks: [
        "01s","08s","05s","04s","06s","03s","10s","02s",
        "01e","08e","05e","04e","06e","03e","10e","02e",
        "01m","09m","05m","04m","06m","03m","07m","02m",
        "01w","08w","05w","04w","11w","03w","10w","02w",
        "01s","04s","03s","02s",
        "01e","05e","03e","02e",
        "01m","04m","03m","02m",
        "01w","04w","03w","02w",
        "01s","02s",
        "01e","02e",
        "01m","03m",
        "01w","02w",
        "01s","01e","01m","01w",
        "01e","01s","01e"
    ]
};

let rachelPicks = {
    name: "rachel",
    medal: "",
    score: 0,
    picks: [
        "01s","09s","05s","13s","06s","14s","07s","02s",
        "01e","08e","12e","04e","06e","03e","10e","02e",
        "01m","09m","12m","04m","11m","03m","10m","02m",
        "16w","","12w","04w","06w","14w","10w","02w",
        "01s","05s","06s","02s",
        "01e","04e","06e","10e",
        "01m","04m","03m","10m",
        "16w","12w","06w","02w",
        "05s","02s",
        "04e","06e",
        "01m","03m",
        "12w","02w",
        "02s","04e","03m","02w",
        "02w","02s","02w"
    ]
};

// let nickPicks = {
//     name: "nick",
//     medal: "",
//     score: 0,
//     picks: [
//         "16s","09s","12s","04s","06s","03s","07s","02s",
//         "01e","08e","05e","13e","06e","03e","07e","02e",
//         "01m","08m","12m","04m","11m","03m","07m","02m",
//         "01w","08w","05w","04w","06w","03w","07w","02w",
//         "09s","12s","03s","02s",
//         "08e","05e","03e","02e",
//         "01m","04m","03m","02m",
//         "01w","04w","03w","02w",
//         "09s","02s",
//         "05e","02e",
//         "04m","02m",
//         "01w","03w",
//         "02s","05e","02m","01w"
//         // "05e","01w","01w"
//     ]
// };

// let lukePicks = {
//     name: "luke",
//     medal: "",
//     score: 0,
//     picks: [
//         "01s","08s","05s","04s","11s","03s","07s","02s",
//         "01e","08e","05e","04e","06e","03e","07e","02e",
//         "01m","09m","05m","04m","11m","03m","10m","02m",
//         "01w","09w","12w","13w","06w","03w","10w","02w",
//         "01s","05s","03s","02s",
//         "08e","05e","06e","07e",
//         "01m","04m","03m","02m",
//         "01w","13w","06w","02w",
//         "01s","03s",
//         "05e","07e",
//         "01m","02m",
//         "01w","02w",
//         "03s","05e","01m","02w"
//         // "05e","01m","05e"
//     ]
// };

// let christinePicks = {
//     name: "christine",
//     medal: "",
//     score: 0,
//     picks: [
//         "01s","08s","05s","04s","06s","03s","07s","02s",
//         "01e","09e","05e","04e","06e","03e","10e","02e",
//         "01m","08m","05m","04m","06m","03m","10m","02m",
//         "01w","09w","12w","04w","06w","03w","07w","02w",
//         "01s","05s","06s","02s",
//         "01e","05e","06e","10e",
//         "01m","05m","06m","02m",
//         "01w","04w","03w","02w",
//         "01s","02s",
//         "05e","10e",
//         "01m","02m",
//         "01w","02w",
//         "01s","05e","01m","02w"
//         // "01s","02w","02w"
//     ]
// };

let leviPicks = {
    name: "levi",
    medal: "",
    score: 0,
    picks: [
        "16s","09s","12s","04s","06s","03s","07s","02s",
        "01e","09e","05e","13e","06e","03e","07e","15e",
        "01m","09m","05m","04m","11m","14m","10m","02m",
        "01w","","05w","13w","11w","03w","10w","02w",
        "09s","04s","06s","02s",
        "01e","13e","06e","15e",
        "09m","04m","11m","02m",
        "01w","05w","03w","02w",
        "04s","02s",
        "01e","06e",
        "04m","02m",
        "01w","02w",
        "01s","01e","02m","02w",
        "02w","02m","02w"
    ]
};

let players = [susanPicks, dennisPicks, mattPicks, katiePicks, 
    noahPicks, davidPicks, jamesPicks, mabryPicks, rachelPicks, 
    paigePicks, reedPicks, calebPicks, aprilPicks, suzyPicks, 
    renPicks, davePicks, dawsonPicks, savannahPicks, leviPicks];

const participants = 19;

let gm1 = {
    team1:"01s",
    team2:"16s",
    winner: "01s",
    fav:[],
    und:[],
    score:0
}

let gm2 = {
    team1:"08s",
    team2:"09s",
    winner: "09s",
    fav:[],
    und:[],
    score:0
}

let gm3 = {
    team1:"05s",
    team2:"12s",
    winner: "12s",
    fav:[],
    und:[],
    score:0
}

let gm4 = {
    team1:"04s",
    team2:"13s",
    winner: "04s",
    fav:[],
    und:[],
    score:0
}

let gm5 = {
    team1:"06s",
    team2:"11s",
    winner: "11s",
    fav:[],
    und:[],
    score:0
}

let gm6 = {
    team1:"03s",
    team2:"14s",
    winner: "14s",
    fav:[],
    und:[],
    score:0
}

let gm7 = {
    team1:"07s",
    team2:"10s",
    winner: "10s",
    fav:[],
    und:[],
    score:0
}

let gm8 = {
    team1:"02s",
    team2:"15s",
    winner: "02s",
    fav:[],
    und:[],
    score:0
}

let gm9 = {
    team1:"01e",
    team2:"16e",
    winner: "01e",
    fav:[],
    und:[],
    score:0
}

let gm10 = {
    team1:"08e",
    team2:"09e",
    winner: "09e",
    fav:[],
    und:[],
    score:0
}

let gm11 = {
    team1:"05e",
    team2:"12e",
    winner: "05e",
    fav:[],
    und:[],
    score:0
}

let gm12 = {
    team1:"04e",
    team2:"13e",
    winner: "13e",
    fav:[],
    und:[],
    score:0
}

let gm13 = {
    team1:"06e",
    team2:"11e",
    winner: "11e",
    fav:[],
    und:[],
    score:0
}

let gm14 = {
    team1:"03e",
    team2:"14e",
    winner: "03e",
    fav:[],
    und:[],
    score:0
}

let gm15 = {
    team1:"07e",
    team2:"10e",
    winner: "07e",
    fav:[],
    und:[],
    score:0
}

let gm16 = {
    team1:"02e",
    team2:"15e",
    winner: "02e",
    fav:[],
    und:[],
    score:0
}

let gm17 = {
    team1:"01m",
    team2:"16m",
    winner: "01m",
    fav:[],
    und:[],
    score:0
}

let gm18 = {
    team1:"08m",
    team2:"09m",
    winner: "08m",
    fav:[],
    und:[],
    score:0
}

let gm19 = {
    team1:"05m",
    team2:"12m",
    winner: "05m",
    fav:[],
    und:[],
    score:0
}

let gm20 = {
    team1:"04m",
    team2:"13m",
    winner: "04m",
    fav:[],
    und:[],
    score:0
}

let gm21 = {
    team1:"06m",
    team2:"11m",
    winner: "11m",
    fav:[],
    und:[],
    score:0
}

let gm22 = {
    team1:"03m",
    team2:"14m",
    winner: "03m",
    fav:[],
    und:[],
    score:0
}

let gm23 = {
    team1:"07m",
    team2:"10m",
    winner: "07m",
    fav:[],
    und:[],
    score:0
}

let gm24 = {
    team1:"02m",
    team2:"15m",
    winner: "02m",
    fav:[],
    und:[],
    score:0
}

let gm25 = {
    team1:"01w",
    team2:"16w",
    winner: "01w",
    fav:[],
    und:[],
    score:0
}

let gm26 = {
    team1:"08w",
    team2:"09w",
    winner: "09w",
    fav:[],
    und:[],
    score:0
}

let gm27 = {
    team1:"05w",
    team2:"12w",
    winner: "12w",
    fav:[],
    und:[],
    score:0
}

let gm28 = {
    team1:"04w",
    team2:"13w",
    winner: "04w",
    fav:[],
    und:[],
    score:0
}

let gm29 = {
    team1:"06w",
    team2:"11w",
    winner: "06w",
    fav:[],
    und:[],
    score:0
}

let gm30 = {
    team1:"03w",
    team2:"14w",
    winner: "03w",
    fav:[],
    und:[],
    score:0
}

let gm31 = {
    team1:"07w",
    team2:"10w",
    winner: "07w",
    fav:[],
    und:[],
    score:0
}

let gm32 = {
    team1:"02w",
    team2:"15w",
    winner: "02w",
    fav:[],
    und:[],
    score:0
}

let gm33 = {
    team1:gm1.winner,
    team2:gm2.winner,
    winner: "01s",
    fav:[],
    und:[],
    score:0
}

let gm34 = {
    team1:gm3.winner,
    team2:gm4.winner,
    winner: "04s",
    fav:[],
    und:[],
    score:0
}

let gm35 = {
    team1:gm5.winner,
    team2:gm6.winner,
    winner: "11s",
    fav:[],
    und:[],
    score:0
}

let gm36 = {
    team1:gm7.winner,
    team2:gm8.winner,
    winner: "02s",
    fav:[],
    und:[],
    score:0
}

let gm37 = {
    team1:gm9.winner,
    team2:gm10.winner,
    winner: "01e",
    fav:[],
    und:[],
    score:0
}

let gm38 = {
    team1:gm11.winner,
    team2:gm12.winner,
    winner: "05e",
    fav:[],
    und:[],
    score:0
}

let gm39 = {
    team1:gm13.winner,
    team2:gm14.winner,
    winner: "03e",
    fav:[],
    und:[],
    score:0
}

let gm40 = {
    team1:gm15.winner,
    team2:gm16.winner,
    winner: "02e",
    fav:[],
    und:[],
    score:0
}

let gm41 = {
    team1:gm17.winner,
    team2:gm18.winner,
    winner: "01m",
    fav:[],
    und:[],
    score:0
}

let gm42 = {
    team1:gm19.winner,
    team2:gm20.winner,
    winner: "05m",
    fav:[],
    und:[],
    score:0
}

let gm43 = {
    team1:gm21.winner,
    team2:gm22.winner,
    winner: "03m",
    fav:[],
    und:[],
    score:0
}

let gm44 = {
    team1:gm23.winner,
    team2:gm24.winner,
    winner: "02m",
    fav:[],
    und:[],
    score:0
}

let gm45 = {
    team1:gm25.winner,
    team2:gm26.winner,
    winner: "01w",
    fav:[],
    und:[],
    score:0
}

let gm46 = {
    team1:gm27.winner,
    team2:gm28.winner,
    winner: "04w",
    fav:[],
    und:[],
    score:0
}

let gm47 = {
    team1:gm29.winner,
    team2:gm30.winner,
    winner: "06w",
    fav:[],
    und:[],
    score:0
}

let gm48 = {
    team1:gm31.winner,
    team2:gm32.winner,
    winner: "02w",
    fav:[],
    und:[],
    score:0
}

let gm49 = {
    team1:gm33.winner,
    team2:gm34.winner,
    winner: "04s",
    fav:[],
    und:[],
    score:0
}

let gm50 = {
    team1:gm35.winner,
    team2:gm36.winner,
    winner: "11s",
    fav:[],
    und:[],
    score:0
}

let gm51 = {
    team1:gm37.winner,
    team2:gm38.winner,
    winner: "01e",
    fav:[],
    und:[],
    score:0
}

let gm52 = {
    team1:gm39.winner,
    team2:gm40.winner,
    winner: "03e",
    fav:[],
    und:[],
    score:0
}

let gm53 = {
    team1:gm41.winner,
    team2:gm42.winner,
    winner: "01m",
    fav:[],
    und:[],
    score:0
}

let gm54 = {
    team1:gm43.winner,
    team2:gm44.winner,
    winner: "02m",
    fav:[],
    und:[],
    score:0
}

let gm55 = {
    team1:gm45.winner,
    team2:gm46.winner,
    winner: "04w",
    fav:[],
    und:[],
    score:0
}

let gm56 = {
    team1:gm47.winner,
    team2:gm48.winner,
    winner: "06w",
    fav:[],
    und:[],
    score:0
}

let gm57 = {
    team1:gm49.winner,
    team2:gm50.winner,
    winner: "",
    fav:[],
    und:[],
    score:0
}

let gm58 = {
    team1:gm51.winner,
    team2:gm52.winner,
    winner: "01e",
    fav:[],
    und:[],
    score:0
}

let gm59 = {
    team1:gm53.winner,
    team2:gm54.winner,
    winner: "01m",
    fav:[],
    und:[],
    score:0
}

let gm60 = {
    team1:gm55.winner,
    team2:gm56.winner,
    winner: "04w",
    fav:[],
    und:[],
    score:0
}

let gm61 = {
    team1:gm60.winner,
    team2:gm58.winner,
    winner: "",
    fav:[],
    und:[],
    score:0
}

let gm62 = {
    team1:gm59.winner,
    team2:gm57.winner,
    winner: "",
    fav:[],
    und:[],
    score:0
}

let gm63 = {
    team1:gm61.winner,
    team2:gm62.winner,
    winner: "",
    fav:[],
    und:[],
    score:0
}

let games = [
    gm1,gm2,gm3,gm4,gm5,gm6,gm7,gm8,
    gm9,gm10,gm11,gm12,gm13,gm14,gm15,gm16,
    gm17,gm18,gm19,gm20,gm21,gm22,gm23,gm24,
    gm25,gm26,gm27,gm28,gm29,gm30,gm31,gm32,
    gm33,gm34,gm35,gm36,gm37,gm38,gm39,gm40,
    gm41,gm42,gm43,gm44,gm45,gm46,gm47,gm48,
    gm49,gm50,gm51,gm52,gm53,gm54,gm55,gm56,
    gm57,gm58,gm59,gm60,gm61,gm62,gm63
];

for (let i=0; i<players.length; i++) {
    for (let j=0; j<games.length; j++) {
        if (players[i].picks[j] === games[j].team1) {
            games[j].fav.push(players[i].name);
        } else if (players[i].picks[j] === games[j].team2) {
            games[j].und.push(players[i].name);
        }
    }
}



// for (let j=0; j<games.length; j++) {
//     let teamAID = games[j].team1;
//     let teamBID = games[j].team2;
//     let teamA = teamAID.charAt(0) + teamAID.charAt(1);
//     let teamB = teamBID.charAt(0) + teamBID.charAt(1);
//     console.log(teamA, teamB);
//     if (teamA < teamB) {
//         for (let i=0; i<games[j].fav.length; i++) {
//             $("#" + teamAID).addClass(games[j].fav[i]);
//         }
//     } else {
//         for (let i=0; i<games[j].fav.length; i++) {
//             $("#" + teamAID).addClass(games[j].fav[i]);
//         }
//     }
// }

// Scores Set to Game

for (let j=0; j<48; j++) {
    console.log(j, "FAV:" + games[j].team1 + games[j].fav, "UND:" + games[j].team2 + games[j].und);

    for (let i=0; i<games[j].fav.length; i++) {
        $("#gm" + (j+1) + games[j].team1).addClass(games[j].fav[i]);
    }

    for (let i=0; i<games[j].und.length; i++) {
        $("#gm" + (j+1) + games[j].team2).addClass(games[j].und[i]);
    }

    if (games[j].winner === games[j].team1) {
        games[j].score = (participants/games[j].fav.length);
    } else if (games[j].winner === games[j].team2) {
        games[j].score = (participants/games[j].und.length);
    }
}

for (let j=48; j<games.length; j++) {

    for (let i=0; i<games[j].fav.length; i++) {
        $("#gm" + (j+1) + games[j].team1).addClass(games[j].fav[i]);
    }

    for (let i=0; i<games[j].und.length; i++) {
        $("#gm" + (j+1) + games[j].team2).addClass(games[j].und[i]);
    }

    if (games[j].winner === games[j].team1) {
        games[j].score = ((participants*2)/games[j].fav.length);
    } else if (games[j].winner === games[j].team2) {
        games[j].score = ((participants*2)/games[j].und.length);
    }
}

// Scores Set to Player

for (let i=0; i<players.length; i++) {
    for (let j=0; j<games.length; j++) {
        if (players[i].picks[j] === games[j].winner) {
            players[i].score = players[i].score + games[j].score;
        }
    }
}

let ordered = players.sort(
    (p1, p2) => (p1.score < p2.score) ? 1 : (p1.score > p2.score) ? -1 : 0);

let leaderRows = "";
ordered.forEach((player) => {
    leaderRows +=
    `<div class="gm20">
        <div class="scores ${player.medal}">
            <div class="${player.name}"></div>
            <div><h3>${player.name}</h3></div>
            <div><h3>${player.score.toFixed(2)}</h3></div>
        </div>
    </div>`
});
document.querySelector(".scoreboard").innerHTML = leaderRows;